package com.jt.tastytown.backend.security.jwt;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            if (authHeader == null || !authHeader.startsWith("Bearer")) {
                filterChain.doFilter(request, response); //lets spring security handle unauthorized accesses
                return;
            }
            ...
            if (userId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                // extracts user obj for extracting his/her roles
                User user = userRepository.findById(userId).orElseThrow();
                // extracts his/her role from the user obj
                var roles = List.of(new SimpleGrantedAuthority(user.getRole().toString()));
                // creates authentication obj which contains the user information along with its roles
                var authentication = new UsernamePasswordAuthenticationToken(user, null, roles);
                // stores the authentication obj inside the Security Context
